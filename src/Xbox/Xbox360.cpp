...
double Xbox360::GetLeftAngle(bool reflectCircle) {
	...
	if (xAxis == 0 && yAxis == 0) {
		...
		angle = math->custom_atan(yAxis / xAxis) + (3 * CUSTOM_PI / 2);
	} else if (yAxis > 0 && xAxis < 0) {
		angle = (CUSTOM_PI / 2) + math->custom_atan(yAxis / xAxis);
	} else if (yAxis < 0 && xAxis > 0) {
		angle = math->custom_atan(yAxis / xAxis) + (3 * CUSTOM_PI / 2);
	} else if (yAxis < 0 && xAxis < 0) {
		angle = math->custom_atan(yAxis / xAxis) + (CUSTOM_PI / 2);
	}
...
double Xbox360::GetLeftMagnitude() {
	return (Limit(
			math->custom_sqrt(
					math->custom_pow(GetLeftX(), 2)
							+ math->custom_pow(GetLeftY(), 2))));
}
...
double Xbox360::GetRightMagnitude() {
	return (Limit(
			math->custom_sqrt(
					math->custom_pow(GetRightX(), 2)
							+ math->custom_pow(GetRightY(), 2))));
}
 ...
 */
bool Xbox360::GetRawButton(uint32_t button) {
 ...
 * @return The value of the axis from -1 to 1.
 */
float Xbox360::GetRawAxis(uint32_t axis) {
